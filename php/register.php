	...
	if(isset($_POST['n']) && isset($_POST['e']) && isset($_POST['a'])
		...
		if($pass != $rpass && $count == 0){
			...
			if($count == 0){
				...
				$msg = '<h1 style="color:green;">Successfully Logged In</h1>'.header("refresh:10;url=index.html"); 
			}else{
...
<center>	  
		<?php
			$register = '<h1>Register Now!</h1><form action = '.$_SERVER['PHP_SELF'].' method=post>'
